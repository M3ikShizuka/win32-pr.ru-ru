---
title: Guide (COM)
description: В этом руководством рассматривается модель COM.
ms.assetid: 9c67dbfd-31ce-4664-a34a-4d26d97e1b1d
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 1fdb6473146d1c30142c24d350261a18149e0198
ms.sourcegitcommit: d75fc10b9f0825bbe5ce5045c90d4045e3c53243
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 09/13/2021
ms.locfileid: "126971351"
---
# <a name="guide-com"></a>Guide (COM)

В этом руководством рассматривается модель COM.

-   [Новые возможности COM](what-s-new-in-com.md)
-   [Модель COM](the-component-object-model.md)
-   [Процессы, потоки и подразделения](processes--threads--and-apartments.md)
-   [COM-клиенты и серверы](com-clients-and-servers.md)
-   [Создание и обработка асинхронных вызовов](making-and-processing-asynchronous-calls.md)
-   [Определение COM-интерфейсов](defining-com-interfaces.md)
-   [Регистрация приложений COM](registering-com-applications.md)
-   [Безопасность в COM](security-in-com.md)
-   [Обработка ошибок в COM](error-handling-in-com.md)
-   [Обработчики COM](com-handlers.md)
-   [Суррогаты библиотеки DLL](dll-surrogates.md)
-   [Моникеров](monikers.md)
-   [События в COM и подключаемых объектах](events-in-com-and-connectable-objects.md)
-   [Реализация диспетчера категорий компонентов](component-categories-manager-implementation.md)

## <a name="related-topics"></a>Связанные темы

<dl> <dt>

[Reference](reference.md)
</dt> </dl>

 

 




