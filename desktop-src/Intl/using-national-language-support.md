---
description: Поддержка национальных языков
ms.assetid: 48d96954-90ca-4a4b-b62d-3dd2348fc61c
title: Поддержка национальных языков
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 88a01664bb8af1f5ad3b740ee815ae99b0b9ee74
ms.sourcegitcommit: d75fc10b9f0825bbe5ce5045c90d4045e3c53243
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 09/13/2021
ms.locfileid: "127254830"
---
# <a name="using-national-language-support"></a>Поддержка национальных языков

Обсуждение использования поддержки национальных языков (NLS) состоит из следующих разделов:

-   [Вызов функций "locale Name"](calling-the--locale-name--functions.md)
-   [Обработка международных доменных имен (IDNs)](handling-internationalized-domain-names--idns.md)
-   [Обработка сортировки в приложениях](handling-sorting-in-your-applications.md)
-   [Сопоставление данных локали](mapping-locale-data.md)
-   [Извлечение и установка сведений о языковых стандартах](retrieving-and-setting-locale-information.md)
-   [Получение сведений о времени и дате](retrieving-time-and-date-information.md)
-   [Работа с эрами японского календаря](era-handling-for-the-japanese-calendar.md)
-   [Использование постоянных данных языкового стандарта](using-persistent-locale-data.md)
-   [Использование Pseudo-Locales для тестирования локализации](using-pseudo-locales-for-localization-testing.md)
-   [Использование нормализации Юникода для представления строк](using-unicode-normalization-to-represent-strings.md)
-   [Работа с пользовательскими языками](working-with-custom-locales.md)
-   [NLS: пример преобразования международного доменного имени (IDN)](nls--internationalized-domain-name--idn--conversion-sample.md)
-   [Пример поддержки междоменного имени в NLS (IDN)](nls--internationalized-domain-name--idn--mitigation-sample.md)
-   [NLS: пример API-интерфейсов на основе имен](nls--name-based-apis-sample.md)
-   [NLS: пример нормализации Юникода](nls--unicode-normalization-sample.md)

 

 



