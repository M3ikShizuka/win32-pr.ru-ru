...
для Windows Server 2008, Windows Vista с пакетом обновления 1 (SP1) и более поздними версиями операционной системы, поддерживается запрос IOCTL для **\_ оптимальной \_ отправки \_ невыполненной работы \_** .

при отправке данных через tcp-соединение с помощью Windowsных сокетов важно обеспечить достаточное количество необработанных данных (отправленных, но еще не подтвержденных) в tcp, чтобы добиться максимальной пропускной способности.
...
в Windows Server 2008, Windows Vista с пакетом обновления 1 (SP1) и более поздних версиях операционной системы, **\_ оптимальное \_ \_ \_ изменение невыполненной работы суперконтроллера** ввода-вывода и [**\_ оптимальные \_ \_ \_ запросы на отработку невыполненной работы sio**](sio-ideal-send-backlog-query.md) поддерживаются на ориентированных на поток сокетах, которые находятся в подключенном состоянии.
