...
|------------------------------------------------|---------|-------------------------------------------------------------------------------------------------------------------------------|
| **\_свойство WPD \_ \_ \_ код операции MTP \_**   | VT \_ UI4 | Обязательный элемент. Идентифицирует код операции, расширенный поставщиком.                                                                    |
| **\_Свойства WPD \_ для \_ \_ параметров операции ext MTP \_** | VT \_ UI4 | Обязательный элемент. Объект **ипортабледевицепропвариантколлектион**, который определяет обязательные параметры для кода операции поставщика. |
...
|--------------------------------------------------------------|------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Свойство WPD " \_ \_ MTP \_ ext \_ передает \_ Общий \_ \_ Размер данных"**     | VT \_ UI8    | Обязательный элемент. Возвращает общий объем данных в байтах, за исключением любых издержек, поступающих от устройства. Если устройство сообщает о неизвестном размере данных (0xFFFFFFFF), драйвер должен повторно вызывать **ReadData** до получения короткого фрагмента данных. |
| **\_свойство WPD \_ . \_ \_ оптимальный \_ \_ Размер буфера \_ перемещения MTP** | VT \_ UI4    | Необязательный параметр. Возвращает оптимальный размер буфера перемещения.                                                                                                                                                                          |
| **\_свойство WPD \_ для \_ \_ контекста перемещения \_ MTP**               | VT \_ LPWSTR | Обязательный элемент. Указывает идентификатор контекста для последующих передач данных.                                                                                                                                                           |
