...
title: учебник. 1. передача Windowsного кодирования мультимедиа
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "127268664"
---
# <a name="tutorial-1-pass-windows-media-encoding"></a>учебник. 1. передача Windowsного кодирования мультимедиа
...
следующая процедура описывает действия по кодированию Windows мультимедийного содержимого в контейнере ASF с помощью однопроходного режима кодирования.
...
-   [Настройка Project](#set-up-the-project)
...
-   [распространенные коды ошибок и Советы отладки](#common-error-codes-and-debugging-tips)
...
-   вы знакомы с Windows кодировщиками мультимедиа, а также с различными типами кодировок, в частности с [постоянным кодированием скорости потока](constant-bit-rate-encoding.md) и [кодированием битовой скорости переменной на основе качества](quality-based-variable-bit-rate--vbr--encoding.md).
...
## <a name="set-up-the-project"></a>Настройка Project
...
**Примечание** .  для Windows 7 Media Foundation предоставляет новую функцию [**мфтранскодежетаудиуутпутаваилаблетипес**](/windows/desktop/api/mfidl/nf-mfidl-mftranscodegetaudiooutputavailabletypes) , которая получает список совместимых типов аудио. Эта функция получает только типы носителей для кодирования CBR.
...
5.  Установите для параметра [**мфпкэй \_ АСФМЕДИАСИНК \_ Автокорректировка \_ скорость**](mfpkey-asfmediasink-autoadjust-bitrate-property.md) значение Variant \_ true, чтобы значения сегмента утечки в мультиплексоре ASF были правильно настроены. сведения об этом свойстве см. в разделе "инициализация мультиплексора и контейнер утечки Параметры" раздела [создание объекта мультиплексора](creating-the-multiplexer-object.md).
    ...
    > [!Note]  
    > Существуют и другие свойства, которые можно задать на глобальном уровне для приемника файла. дополнительные сведения см. в разделе "настройка объекта контентинфо с кодировщиком Параметры" раздела [свойства параметра в объекте контентинфо](setting-properties-in-the-contentinfo-object.md).
...
далее предстоит создать топологию с частичным кодированием путем создания узлов топологии для источника мультимедиа, необходимых Windows кодировщиков мультимедиа и приемника файлов ASF. После добавления узлов топологии вы подключаете узлы источника, преобразования и приемника. Перед добавлением узлов топологии необходимо создать пустой объект Topology, вызвав [**мфкреатетопологи**](/windows/desktop/api/mfidl/nf-mfidl-mfcreatetopology).
...
конвейер Media Foundation не вставляет требуемые Windows кодировщики мультимедиа автоматически для потоков, которые необходимо кодировать. Приложение должно добавить кодировщики вручную. Для этого перечислите потоки в профиле ASF, которые были созданы на шаге, описанном в разделе "Создание объекта профиля ASF" этого руководства. Для каждого потока в источнике и соответствующего потока в профиле создайте экземпляр требуемых кодировщиков. Для этого шага требуется указатель на объект активации для приемника файла, созданного на шаге, описанном в подразделе "Создание приемника файлов ASF" этого руководства.
...
-   откройте файл в проигрыватель Windows Media и проверьте качество кодирования.
...
## <a name="common-error-codes-and-debugging-tips"></a>распространенные коды ошибок и Советы отладки
...
-   Сеанс мультимедиа возвращает \_ \_ из состояния события MF E неподдерживаемый \_ \_ тип D3D.
    ...
    эта ошибка возвращается, если тип носителя источника указывает смешанный режим чередования, который не поддерживается Windows кодировщика видео мультимедиа. Если для типа сжатого видео выбрано значение использовать прогрессивный режим, то конвейер должен использовать преобразование «de-чередование». Поскольку конвейеру не удается найти совпадение (обозначено этим кодом ошибки), необходимо вручную вставить дезагруженный обработчик видео (перекодировать видеопроцессор) между узлами декодера и кодировщика.

-   Сеанс мультимедиа возвращает E \_ INVALIDARG в состоянии события.

    эта ошибка возвращается, если атрибуты типа носителя источника несовместимы с атрибутами типа выходных данных, заданными в кодировщике Windows media.
