---
description: В этом разделе описываются компоненты Media Foundation, поддерживающие форматы MPEG-4, включая видео H. 264, аудио AAC, а также форматы файлов MP4 и 3GP.
ms.assetid: a8fa05c6-74b4-408e-8390-9dcbafa012f1
title: Поддержка MPEG-4 в Media Foundation
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 054565a4d139fe70ad7736044c3a7ff708db5023
ms.sourcegitcommit: d75fc10b9f0825bbe5ce5045c90d4045e3c53243
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 09/13/2021
ms.locfileid: "127363725"
---
# <a name="mpeg-4-support-in-media-foundation"></a>Поддержка MPEG-4 в Media Foundation

В этом разделе описываются компоненты Media Foundation, поддерживающие форматы MPEG-4, включая видео H. 264, аудио AAC, а также форматы файлов MP4 и 3GP.

-   [**Декодер AAC**](aac-decoder.md)
-   [**Кодировщик AAC**](aac-encoder.md)
-   [**Декодер видео H. 264**](h-264-video-decoder.md)
-   [**Кодировщик видео H. 264**](h-264-video-encoder.md)
-   [**Видеодекодер (часть 2) памяти MPEG4**](mpeg4part2videodecoder.md)
-   [Приемник файлов MPEG-4](mpeg-4-file-sink.md)
-   [Источник файла MPEG-4](mpeg-4-file-source.md)

## <a name="related-topics"></a>Связанные темы

<dl> <dt>

[Поддерживаемые форматы мультимедиа в Media Foundation](supported-media-formats-in-media-foundation.md)
</dt> <dt>

[Media Foundationое программное руководством](media-foundation-programming-guide.md)
</dt> </dl>

 

 



