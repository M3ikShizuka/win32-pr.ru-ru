---
description: Атрибуты Media Foundation
ms.assetid: 445fc879-3c9e-409d-8d05-ecd1ff9afc19
title: Атрибуты Media Foundation
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 29e9283562ef3cd5cc8e52e64f8cd4bfdbc8658d
ms.sourcegitcommit: d75fc10b9f0825bbe5ce5045c90d4045e3c53243
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 09/13/2021
ms.locfileid: "127579919"
---
# <a name="media-foundation-attributes"></a>Атрибуты Media Foundation

В этом разделе перечислены все идентификаторы GUID атрибутов, определенные для Media Foundation, упорядоченные по объектам, к которым они применяются.

-   [Алфавитный список атрибутов Media Foundation](alphabetical-list-of-media-foundation-attributes.md)
-   [Атрибуты ASF](asf-attributes.md)
-   [Атрибуты модуля подготовки звука](audio-renderer-attributes.md)
-   [Атрибуты потока байтов](byte-stream-attributes.md)
-   [Запись атрибутов устройства](capture-device-attributes.md)
-   [Атрибуты подсистемы захвата](capture-engine-attributes.md)
-   [Атрибуты событий](event-attributes.md)
-   [Атрибуты Евр](enhanced-video-renderer-attributes.md)
-   [Атрибуты обработчика мультимедиа](media-engine-attributes.md)
-   [Атрибуты сеанса мультимедиа](media-session-attributes.md)
-   [Атрибуты типа мультимедиа](media-type-attributes.md)
-   [Атрибуты дескриптора представления](presentation-descriptor-attributes.md)
-   [Примеры атрибутов](sample-attributes.md)
-   [Атрибуты модуля записи приемника](sink-writer-attributes.md)
-   [Атрибуты модуля чтения исходного кода](source-reader-attributes.md)
-   [Атрибуты потока](stream-attributes.md)
-   [Атрибуты дескриптора потока](stream-descriptor-attributes.md)
-   [Атрибуты топологии](topology-attributes.md)
-   [Атрибуты узла топологии](topology-node-attributes.md)
-   [Атрибуты перекодирования](transcode-attributes.md)
-   [Атрибуты преобразования](transform-attributes.md)

## <a name="related-topics"></a>Связанные темы

<dl> <dt>

[**имфаттрибутес**](/windows/desktop/api/mfobjects/nn-mfobjects-imfattributes)
</dt> <dt>

[Справочник по программированию в Media Foundation](media-foundation-programming-reference.md)
</dt> <dt>

[Атрибуты и свойства](attributes-and-properties.md)
</dt> </dl>

 

 



