...
кроме того, стабилизации для видео DSP поддерживает создание экземпляров с помощью среда выполнения Windows в качестве расширения носителя Windows. Он определен в [**Windows. Media. Видеоеффектс**](/uwp/api/Windows.Media.VideoEffects?view=winrt-19041)и его полное имя — "Windows. Media. Видеоеффектс. Видеостабилизатион ".
