---
title: Общие элементы
description: Общие элементы
ms.assetid: 4dc2da69-58d9-4dee-bdf5-74339ceff1c5
keywords:
- Платформа текстовых служб (TSF), общие элементы
- TSF (платформа текстовых служб), общие элементы
- текстовые службы, общие элементы
- Приложения с поддержкой TSF, общие элементы
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 630327dce70f6fee292eaa236baa99f589da3bd6
ms.sourcegitcommit: d75fc10b9f0825bbe5ce5045c90d4045e3c53243
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 09/13/2021
ms.locfileid: "127243709"
---
# <a name="common-elements"></a>Общие элементы

Следующие элементы используются и в [приложениях с поддержкой TSF](architecture.md) , и в текстовых службах.

-   [Диспетчер потоков](thread-manager.md)
-   [Идентификаторы клиентов](client-identifiers.md)
-   [Диспетчер документов](document-manager.md)
-   [Изменение контекстов](edit-contexts.md)
-   [Диапазоны](ranges.md)
-   [Свойства](properties.md)
-   [Секции](compartments.md)
-   [Композиции](compositions.md)

 

 




