---
description: собственный кодек XR JPEG доступен через компонент обработки изображений Windows (WIC). Формат JPEG XR, который поддерживает кодек, предназначен для бытовой цифровой фотографии.
...
ms.locfileid: "122480750"
---
# <a name="jpeg-xr-codec-overview"></a>Общие сведения о коде XR JPEG

собственный кодек XR JPEG доступен через компонент обработки изображений Windows (WIC). Формат JPEG XR, который поддерживает кодек, предназначен для бытовой цифровой фотографии.
...
### <a name="image-quality-settings"></a>качество изображения Параметры
...
| Значение | Описание | 
|-------|-------------|
| 3 | Кодировка 4:4:4. Сохраняет полное разрешение чрома. | 
| 2 | Кодировка 4:2:2. Разрешение чрома — 1/2 для разрешения освещенности. | 
| 1 | Кодировка 4:2:0. Разрешение чрома — 1/4 для разрешения освещенности. | 
| 0 | Кодирование 4:0:0. Отменяет все значения чрома и сохраняет только светимость.<blockquote>[!Note]<br />Этот режим не рекомендуется, поскольку кодек использует немного измененное определение светимости для повышения производительности. Вместо этого лучше преобразовать изображение в монохромную перед кодированием.</blockquote><br /> | 
