...
\_МНОГОпотоковый флаг D3DCREATE иногда используется для подготовки к просмотру в одном потоке и создании ресурсов в других потоках. Этот флаг игнорируется на Xbox 360, и его следует избегать на Windows. при выборе этого флага в Windows указывает, что D3D тратит значительное время на синхронизацию, что замедляет поток отрисовки.
...
Если для создания потоков используется функция [**CreateThread**](/windows/win32/api/processthreadsapi/nf-processthreadsapi-createthread) , то среда выполнения C/C++ (CRT) не будет правильно инициализирована на Windows. Вместо этого рекомендуется использовать функцию CRT [**\_ бегинсреадекс**](https://msdn.microsoft.com/library/ms397047(v=VS.70).aspx) .
...
На Xbox 360 необходимо явным образом назначить программные потоки конкретному аппаратному потоку с помощью **кссетсреадпроцессор**. В противном случае все дочерние потоки будут оставаться в том же аппаратном потоке, что и родительский. на Windows можно использовать [**сетсреадаффинитимаск**](/windows/win32/api/winbase/nf-winbase-setthreadaffinitymask) для обеспечения строгого предложения операционной системе о том, на каких аппаратных потоках должен выполняться поток. как правило, этот метод следует избегать в Windows, поскольку неизвестно, какие другие процессы могут быть запущены в системе. обычно лучше разрешить планировщику Windows назначать потоки в бездействующие аппаратные потоки.
...
другая причина скрытой синхронизации — D3DCREATE \_ многопоточность, что приводит к тому, что D3D на Windows использует синхронизацию во многих операциях. (Этот флаг игнорируется на Xbox 360.)
...
## <a name="windows-vista-and-windows-7-improvements"></a>Windows улучшения Vista и Windows 7
...
-   [примитивы синхронизации, новые для Windows Vista](/archive/msdn-magazine/2007/june/concurrency-synchronization-primitives-new-to-windows-vista)
