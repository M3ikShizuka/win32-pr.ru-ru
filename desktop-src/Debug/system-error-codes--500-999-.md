...
база данных SAM на Windows сервере значительно не синхронизирована с копией на контроллере домена. Требуется полная синхронизация.
...
Сбой API NtCreateFile. эта ошибка никогда не должна возвращаться в приложение. это заполнитель для перенаправления Windows Lan Manager для использования в внутренних подпрограммых сопоставления ошибок.
...
неверная конфигурация сервера Windows.
...
{Windows уведомление об оценке} период ознакомительного использования этой установки Windows истек. Эта система завершит работу через 1 час. чтобы восстановить доступ к этой установке Windows, обновите эту установку с помощью лицензированного дистрибутива этого продукта.
...
{Недопустимое перемещение системной библиотеки DLL} Системная библиотека DLL% HS была перемещена в память. Приложение не будет работать должным образом. перемещение произошло, так как библиотека dll% hs занимала диапазон адресов, зарезервированный для Windows системных библиотек dll. Поставщик, предоставляющий библиотеку DLL, должен быть обращен к новой библиотеке DLL.
...
{сбой Подключение на основном транспорте} Предпринята попытка подключения к удаленному серверу% HS на основном транспорте, но подключение не выполнено. Компьютер смог подключиться к дополнительному транспорту.
