---
description: 'В сети DDE используется следующая структура:'
ms.assetid: 1cbfd363-4fe4-452a-950d-637e9b96ca16
title: Структуры сетевого DDE
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 81780ad389e2e18c28afd88b7056067861667483
ms.sourcegitcommit: d75fc10b9f0825bbe5ce5045c90d4045e3c53243
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 09/13/2021
ms.locfileid: "127458173"
---
# <a name="network-dde-structures"></a>Структуры сетевого DDE

\[Сетевой DDE больше не поддерживается. Nddeapi.dll имеется в Windows Vista, но все вызовы функций возвращают ндде \_ не \_ реализованы.\]

В сети DDE используется следующая структура:

-   [**нддешареинфо**](nddeshareinfo-str.md)

 

 



