...
| Атрибут | Описание | 
|-----------|-------------|
| ининвертединдекс | Общедоступный. Необязательный параметр. Указывает, должно ли значение свойства храниться в инвертированном индексе. Это позволяет конечным пользователям выполнять полнотекстовые запросы по значениям этого свойства. Значение по умолчанию — «false». | 
