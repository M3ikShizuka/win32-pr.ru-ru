---
title: атрибут MS-KDS-ПОДПРОГРАММ-param
description: Параметры алгоритма формирования ключа.
ms.assetid: 66f3273d-f963-48e6-b0dc-c8bbebe4d2d3
ms.tgt_platform: multiple
keywords:
- Схема AD атрибута Active-KDS-ПОДПРОГРАММ-param
- Схема AD атрибута Мскдс-Кдфпарам
topic_type:
- apiref
api_name:
- ms-Kds-KDF-Param
api_type:
- Schema
ms.topic: reference
ms.date: 05/31/2018
ms.openlocfilehash: 4e23b47224644c2271fcc09070edac47f57afcd398b81ed8193a0f8592d98666
ms.sourcegitcommit: e6600f550f79bddfe58bd4696ac50dd52cb03d7e
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 08/11/2021
ms.locfileid: "119762974"
---
# <a name="ms-kds-kdf-param-attribute"></a>атрибут MS-KDS-ПОДПРОГРАММ-param

Параметры алгоритма формирования ключа.



| Ввод | Значение |
|-------------------|-------------------------------------------------------|
| CN                | MS-KDS-ПОДПРОГРАММ-param                                      |
| LDAP-отображаемое имя | Мскдс — Кдфпарам                                        |
| Размер              | \-                                                    |
| Привилегия обновления  | \-                                                    |
| Частота обновления  | \-                                                    |
| Attribute-Id      | 1.2.840.113556.1.4.2170                               |
| System-ID — GUID    | 8a800772-f4b8-154f-b41c-2e4271eff7a7                  |
| Синтаксис            | [**Object(ссылка_на_реплику)**](s-object-replica-link.md) |



## <a name="implementations"></a>Варианты реализации решения

-   [**Windows Server 2012**](#windows-server-2012)

## <a name="windows-server-2012"></a>Windows Server 2012



| Ввод | Значение |
|------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------|
| Идентификатор ссылки                | \-                                                                                                                                                  |
| MAPI-Id                | \-                                                                                                                                                  |
| System-Only            | Неверно                                                                                                                                               |
| Является однозначным       | Верно                                                                                                                                                |
| Индексируется             | Неверно                                                                                                                                               |
| В глобальном каталоге      | Неверно                                                                                                                                               |
| NT-Security-дескриптор | О:БАГ: BAD: S:                                                                                                                                        |
| Range-Lower            | \-                                                                                                                                                  |
| Range-Upper            | \-                                                                                                                                                  |
| Search-Flags           | 0x00000280                                                                                                                                          |
| System-Flags           | 0x00000010                                                                                                                                          |
| Классы, используемые в        | [**MS-KDS-Prov-Серверконфигуратион**](c-mskds-provserverconfiguration.md)<br/> [**MS-KDS-Prov-Руткэй**](c-mskds-provrootkey.md)<br/> |



 

 





