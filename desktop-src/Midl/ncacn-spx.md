...
при использовании транспортного **нкакн \_ spx** имя сервера совпадает с именем 32-разрядного Windows. Однако поскольку имена распределяются по протоколам Novell, они должны соответствовать соглашениям об именовании Novell. Если имя сервера не является допустимым именем Novell, серверы не смогут создавать конечные точки с помощью транспорта **нкакн \_ SPX** . Ниже приведен неполный список символов, запрещенных в именах серверов Novell.

" \* + . / : ; < = >? \[ \] \\ \|

Транспорт **нкакн \_ SPX** не поддерживается версией NWLink, предоставляемой в MS Client 3,0.

для 16-разрядных Windows клиентских приложений, использующих транспортный протокол **\_ spx нкакн** , требуется, чтобы файл Nwipxspx.dll был установлен для запуска в подсистеме WOW. Чтобы получить этот файл, обратитесь в Novell.
