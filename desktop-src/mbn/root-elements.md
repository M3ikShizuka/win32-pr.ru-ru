---
description: Элементы схемы профиля Mobile широкополосной сети v4
MS-HAID: WWAN\_profile\_v4.root\_elements
MSHAttr:
- PreferredSiteName:MSDN
- PreferredLib:/library/windows/desktop
title: Элементы схемы профиля Mobile широкополосной сети v4
ms.topic: reference
ms.date: 05/31/2018
ms.assetid: a322bf35-1c63-4c4f-a646-ee862fe83c7a
api_name: ''
api_type: ''
api_location: ''
topic_type:
- kbArticle
ms.openlocfilehash: a3197339b755db93ac36d6d0cd8ba163ee255a67c2473a1ab650c73b1667d0e5
ms.sourcegitcommit: e6600f550f79bddfe58bd4696ac50dd52cb03d7e
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 08/11/2021
ms.locfileid: "119449894"
---
# <a name="span-idwwan_profile_v4root_elementsspanmobile-broadband-profile-schema-v4-elements"></a><span id="WWAN_profile_v4.root_elements"></span>Элементы схемы профиля Mobile широкополосной сети v4

В следующем списке перечислены допустимые иерархии для этой схемы, начиная с самого внешнего элемента (Document) в верхней части.

-   [MBNProfileExt](element-mbnprofileext.md)
    -   [Имя](element-name.md)
    -   [Описание](element-description.md)
    -   [иконфилепас](element-iconfilepath.md)
    -   [IsDefault](element-isdefault.md)
    -   [профилекреатионтипе](element-profilecreationtype.md)
    -   [субскриберид](element-subscriberid.md)
    -   [симикЦид](element-simiccid.md)
    -   [HomeProviderName](element-homeprovidername.md)
    -   [аутоконнектонинтернет](element-autoconnectoninternet.md)
    -   [ConnectionMode](element-connectionmode.md)
    -   [Контекст](element-context.md)
        -   [AccessString](element-accessstring.md)
        -   [UserLogonCred](element-userlogoncred.md)
            -   [UserName](element-username.md)
            -   [игнорепассворд](element-ignorepassword.md)
            -   [Пароль](element-password.md)
        -   [Сжатие](element-compression.md)
        -   [AuthProtocol](element-authprotocol.md)
        -   [IPType](element-iptype.md)
    -   [DataRoamingPartners](element-dataroamingpartners.md)
        -   [Поставщик](element-provider.md)
    -   [пурпосеграупс](element-purposegroups.md)
        -   [пурпосеграупгуид](element-purposegroupguid.md)
    -   [профилекондитионедон](element-profileconditionedon.md)
        -   [целлуларкласс](element-cellularclass.md)
        -   [ратаппликабилити](element-ratapplicability.md)
        -   [роамаппликабилити](element-roamapplicability.md)
        -   [IMSI](element-imsi.md)
        -   [ивланаппликабилити](element-iwlanapplicability.md)
    -   [испровисионингпрофиле](element-isprovisioningprofile.md)
    -   [апнид](element-apnid.md)
    -   [админенабле](element-adminenable.md)
    -   [админроамконтрол](element-adminroamcontrol.md)
    -   [исексклусиветусер](element-isexclusivetoother.md)
    -   [ислонгстандингаддитионалпдпконтекстпрофиле](element-islongstandingadditionalpdpcontextprofile.md)
    -   [ммсконфигуратион](element-mmsconfiguration.md)
        -   [MmscUrl](element-mmscurl.md)
        -   [MmscPort](element-mmscport.md)
        -   [MmsMaximumMessageSize](element-mmsmaximummessagesize.md)
-   [ModemDMConfigProfile](element-modemdmconfigprofile.md)
    -   [Имя](element-1-name.md)
    -   [симикЦид](element-1-simiccid.md)
    -   [апнид](element-1-apnid.md)
    -   [оемконнектионид](element-oemconnectionid.md)
    -   [роамаппликабилити](element-1-roamapplicability.md)
    -   [Контекст](element-1-context.md)
        -   [AccessString](element-1-accessstring.md)
        -   [UserLogonCred](element-1-userlogoncred.md)
            -   [UserName](element-1-username.md)
            -   [игнорепассворд](element-1-ignorepassword.md)
            -   [Пароль](element-1-password.md)
        -   [Сжатие](element-1-compression.md)
        -   [AuthProtocol](element-1-authprotocol.md)
        -   [IPType](element-1-iptype.md)
    -   [админенабле](element-1-adminenable.md)
    -   [админроамконтрол](element-1-adminroamcontrol.md)
    -   [профилекреатионтипе](element-1-profilecreationtype.md)

 

 



