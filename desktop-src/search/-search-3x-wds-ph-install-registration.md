...
title: установка и регистрация обработчиков протоколов (Windows поиск)
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "122880123"
---
# <a name="installing-and-registering-protocol-handlers-windows-search"></a>установка и регистрация обработчиков протоколов (Windows поиск)
...
Windows Поиск использует URL-адреса для уникальной идентификации элементов в иерархии источника данных оболочки. URL-адрес, который является первым узлом в иерархии, называется [корнем поиска](-search-3x-wds-extidx-csm-searchroots.md). Windows Поиск начнет индексироваться в корне поиска, запрашивая, что обработчик протокола перечислит дочерние ссылки для каждого URL-адреса.
...
| &lt;путь&gt;     | Определяет иерархию хранилища, где каждая косая черта ("/") является разделителем между именами папок.                                                                                                            |
...
индексатор поиска Windows обрезает окончательную косую черту по url-адресам. В результате нельзя полагаться на существование последней косой черты для определения каталога и элемента. Обработчик протокола должен иметь возможность обрабатывать этот синтаксис URL-адреса. Убедитесь, что имя протокола, выбранное для обнаружения источника данных оболочки, не конфликтует с текущими. Мы рекомендуем использовать такое соглашение об именовании: `companyName.scheme` .
...
4.  зарегистрируйте обработчик протокола с помощью Windows поиска. В следующем примере <Protocol Name> — это имя самого протокола, например File, MAPI и т. д.
