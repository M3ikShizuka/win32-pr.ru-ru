---
description: Windows В Vista используются файлы эскизов, относящиеся к файлам, чем в предыдущих версиях Windows.
...
ms.locfileid: "127247933"
---
# <a name="thumbnail-handlers"></a>Обработчики эскизов

Windows В Vista используются файлы эскизов, относящиеся к файлам, чем в предыдущих версиях Windows. Windows Vista использует их во всех представлениях, в диалоговых окнах и для любого типа файла, который их предоставляет. Другие приложения также могут использовать эскиз. Отображение эскиза также изменилось. теперь доступен непрерывный спектр размеров, выбираемых пользователем, а не дискретные размеры, такие как значки и эскизы, предоставляемые в Windows XP.
...
эскизы 32-разрядного разрешения и размера, такого как 256x256 пикселей, часто используются в пользовательском интерфейсе Windows Vista. Владельцы форматов файлов должны быть готовы к отображению эскизов с таким размером. Они также должны предоставлять нестатические изображения для эскизов, отражающих содержимое конкретного файла. Например, эскиз текстового файла должен показывать миниатюрную версию документа, включая его текст.

Интерфейс [**исумбнаилпровидер**](/windows/desktop/api/Thumbcache/nn-thumbcache-ithumbnailprovider) был введен, чтобы сделать эскиз более простым и простым, чем раньше, когда вместо этого было бы использовано [**иекстрактимаже**](/windows/desktop/api/shobjidl_core/nn-shobjidl_core-iextractimage) . обратите внимание, что существующий код, использующий **иекстрактимаже** , по-прежнему действителен в Windows Vista. Однако **иекстрактимаже** не поддерживается в области **сведений** .
...
при необходимости эскиза Windows сначала проверяет кэш эскизов для изображения. Средство извлечения эскизов вызывается, если изображение не найдено в кэше. Он также вызывается, когда время последнего изменения образа позже, чем у копии в кэше.
...
Перекрытия для эскизов должны предоставляться только с помощью этого механизма и применяться с помощью Windows. Не применяйте перекрытия самостоятельно.

## <a name="thumbnail-adornments"></a>Украшения эскизов

Декоративные элементы, такие как тени, применяются к эскизам на основе выбранной темы пользователя. Декоративные элементы предоставляются Windows; не создавайте их самостоятельно. Windows может изменить внешний вид определенных оформлений в любое время, поэтому, если вы являетесь владельцем вас, вы рискуете не синхронизироваться с системой. Эскизы могут быть в начале или на месте.
