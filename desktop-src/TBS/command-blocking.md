...
Для команд TPM версии 2,0 логика для блокировки инвертирована; в нем используется список разрешенных команд. Эта логика автоматически блокирует команды, которые не были известны при первом внесении списка. когда команды добавляются в спецификацию TPM после доставки версии Windows, эти новые команды автоматически блокируются. Только обновление реестра добавит эти новые команды в список разрешенных команд.

начиная с Windows 10 1809 (Windows Server 2019) разрешенные команды TPM 2,0 больше не могут управляться с помощью параметров реестра. для этих Windows 10 версий разрешенные команды TPM 2,0 исправлены в драйвере tpm. Команды TPM 1,2 по-прежнему могут блокироваться и разблокированы с помощью изменений в реестре. 
