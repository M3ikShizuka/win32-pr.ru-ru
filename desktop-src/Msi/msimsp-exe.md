...
Необязательный параметр. , За которым следует путь к временной папке. Расположение по умолчанию:% TMP% \\ ~ PCW \_ tmp. tmp \\ .
...
Необязательный параметр. , За которым следует путь к файлу журнала, описывающий процесс создания исправления и ошибки. Дополнительные сведения см. в разделе [возвращаемые значения для уикреатепатчпаккаже](return-values-for-uicreatepatchpackage.md).
...
Необязательный параметр. , За которым следует путь к файлу журнала, описывающий процесс создания исправления и ошибки. Этот параметр записывает данные о производительности в файл журнала. Для этого параметра требуется версия 4,0 Patchwiz.dll.
...
Необязательный параметр. Отображает диалоговое окно при успешном завершении создания исправления.
