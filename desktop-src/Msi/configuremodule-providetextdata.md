...
Средство должно вернуть S \_ OK и предоставить соответствующий текст настройки в конфигдата. Клиентское средство отвечает за выделение данных, но Мержемод. дллис отвечает за освобождение памяти. Этот аргумент должен быть объектом **BSTR** . **ЛПКВСТР** не принимается.
