...
Windows 8 отключает зеркальные драйверы стандартных Windows 2000 для модели видеодрайверов (ксддм). DXGI 1,2 предоставляет в качестве альтернативы API дублирования рабочего стола. API дублирования настольных систем обеспечивает удаленный доступ к образу рабочего стола для сценариев совместной работы.
...
начиная с модели драйвера wddm 1,2, планировщик wddm может запланировать выполнение задач приложения на более тонком уровне детализации Windows с помощью графического процессора. DXGI 1,2 позволяет определить уровни гранулярности при вытеснении GPU.
