...
-   **Асинхронная**. В асинхронном режиме скрипт вызывает один из девяти асинхронных методов и немедленно возвращает результат. То есть, как только вызывается асинхронный метод, сценарий возобновляет выполнение следующей строки кода. Чтобы использовать асинхронный метод, сценарий должен сначала создать объект [**свбемсинк**](swbemsink.md) и специальную подпрограммы, называемую обработчиком событий. Инструментарий WMI выполняет асинхронную операцию и уведомляет скрипт, вызывая подпрограммы обработчика событий после завершения операции.
