---
title: Функции ВГЛ
description: Ниже приведены функции ВГЛ, связанные с OpenGL.
ms.assetid: 52053370-d88b-4faf-bdcd-4663c6d5270d
keywords:
- OpenGL, функции ВГЛ
- ВГЛ Справочник по OpenGL, функции
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 150871ed1e059a5a7711c5921c43860a309c4367
ms.sourcegitcommit: d75fc10b9f0825bbe5ce5045c90d4045e3c53243
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 09/13/2021
ms.locfileid: "127067429"
---
# <a name="wgl-functions"></a>Функции ВГЛ

Ниже приведены функции ВГЛ, связанные с OpenGL:

-   [**вглкопиконтекст**](/windows/desktop/api/wingdi/nf-wingdi-wglcopycontext)
-   [**вглкреатеконтекст**](/windows/desktop/api/wingdi/nf-wingdi-wglcreatecontext)
-   [**вглкреателайерконтекст**](/windows/desktop/api/wingdi/nf-wingdi-wglcreatelayercontext)
-   [**вглделетеконтекст**](/windows/desktop/api/wingdi/nf-wingdi-wgldeletecontext)
-   [**вглдескрибелайерплане**](/windows/desktop/api/wingdi/nf-wingdi-wgldescribelayerplane)
-   [**вглжеткуррентконтекст**](/windows/desktop/api/wingdi/nf-wingdi-wglgetcurrentcontext)
-   [**вглжеткуррентдк**](/windows/desktop/api/wingdi/nf-wingdi-wglgetcurrentdc)
-   [**вглжетлайерпалеттинтриес**](/windows/desktop/api/wingdi/nf-wingdi-wglgetlayerpaletteentries)
-   [**вглжетпрокаддресс**](/windows/desktop/api/wingdi/nf-wingdi-wglgetprocaddress)
-   [**вглмакекуррент**](/windows/desktop/api/wingdi/nf-wingdi-wglmakecurrent)
-   [**вглреализелайерпалетте**](/windows/desktop/api/wingdi/nf-wingdi-wglrealizelayerpalette)
-   [**вглсетлайерпалеттинтриес**](/windows/desktop/api/wingdi/nf-wingdi-wglsetlayerpaletteentries)
-   [**вглшарелистс**](/windows/desktop/api/wingdi/nf-wingdi-wglsharelists)
-   [**вглсваплайербуфферс**](/windows/desktop/api/wingdi/nf-wingdi-wglswaplayerbuffers)
-   [**вглусефонтбитмапс**](/windows/desktop/api/wingdi/nf-wingdi-wglusefontbitmapsa)
-   [**вглусефонтаутлинес**](/windows/desktop/api/wingdi/nf-wingdi-wglusefontoutlinesa)

 

 




