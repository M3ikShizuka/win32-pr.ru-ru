...
description: в этом примере сценария показано, как создать задачу, которая выполняется Блокнот в определенное время.
...
ms.locfileid: "127264936"
---
# <a name="time-trigger-example-scripting"></a>Пример триггера времени (сценарии)

в этом примере сценария показано, как создать задачу, которая выполняется Блокнот в определенное время. задача содержит триггер на основе времени, указывающий начальную границу для активации задачи, исполняемое действие, которое выполняется Блокнот, и конечную границу, которая деактивирует задачу.
...
6.  Зарегистрируйте задачу с помощью метода [**таскфолдер. регистертаскдефинитион**](taskfolder-registertaskdefinition.md) . в этом примере задача начнется Блокнот в текущее время плюс 30 секунд.

в следующем примере на языке VBScript показано, как запланировать выполнение задачи Блокнот 30 секунд после регистрации задачи.
