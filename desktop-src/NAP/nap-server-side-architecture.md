...
архитектура серверной платформы NAP использует компьютеры с Windows server 2008. на следующем рисунке показана архитектура поддержки на стороне сервера для платформы NAP, состоящая из точек принудительного применения nap на основе Windows и сервера политики работоспособности nap.
...
-   Служба сервера политики сети (NPS)
    ...
    Получает сообщение Access-Request RADIUS, извлекает Ссох и передает его компоненту сервера администрирования NAP. служба NPS предоставляется с Windows Server 2008.
...
-   четыре серверных компонента защиты доступа к сети (уровень SHV, сервер администрирования nap, служба NPS и уровень NAP ES на точках принудительного применения nap на основе Windows).
...
в состав Windows Server 2008 включена защита доступа к сети (NAP):

-   IPsec NAP ES для обмена данными с защитой IPsec.

    для подключения, защищенного IPsec, это центр регистрации работоспособности (HRA), компьютер с Windows Server 2008 и службы IIS (IIS), который получает сертификаты работоспособности от центра сертификации (цс) для соответствующих компьютеров, передает сведения о состоянии работоспособности клиента nap серверу политики работоспособности nap.
