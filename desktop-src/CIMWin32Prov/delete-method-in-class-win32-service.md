...
-   удаление базовой службы Windows 2000 (например, DHCP) с помощью инструментария WMI удаляет записи реестра для этой службы, но не удаляет ярлык из меню "администрирование" или удаляет службу из мастера Windows компонентов. Это может запутать всех, кто пытается определить, как настроен компьютер.

    Например, если удалить службу DHCP с помощью скрипта WMI, служба DHCP больше не будет отображаться в оснастке "службы". однако нефункциональный ярлык консоли DHCP остается в меню "администрирование", и при запуске мастера Windows компонентов он указывает на то, что служба DHCP установлена.
