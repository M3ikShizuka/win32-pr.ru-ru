---
description: В этом разделе содержатся справочные сведения о макросах, предоставляемых Direct3D.
ms.assetid: cabb715a-2a6a-4d56-8113-5e5ed26fc107
title: Макросы (Direct3D 9)
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 99a957ca9d15784e995ec3bd145cc55387151797
ms.sourcegitcommit: d75fc10b9f0825bbe5ce5045c90d4045e3c53243
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 09/13/2021
ms.locfileid: "127174783"
---
# <a name="macros-direct3d-9"></a>Макросы (Direct3D 9)

В этом разделе содержатся справочные сведения о макросах, предоставляемых Direct3D.

-   [**D3DCLIPPLANEn**](d3dclipplanen.md)
-   [**D3DCOLOR \_ ARGB**](d3dcolor-argb.md)
-   [**D3DCOLOR \_ айув**](d3dcolor-ayuv.md)
-   [**D3DCOLOR \_ колорвалуе**](d3dcolor-colorvalue.md)
-   [**D3DCOLOR \_ RGBA**](d3dcolor-rgba.md)
-   [**D3DCOLOR \_ ксргб**](d3dcolor-xrgb.md)
-   [**D3DCOLOR \_ ксюв**](d3dcolor-xyuv.md)
-   [**D3DDECL \_ конец**](d3ddecl-end.md)
-   [**D3DFVF \_ текскурдсизен**](d3dfvf-texcoordsizen.md)
-   [**D3DGETDATA \_ запись на диск**](d3dgetdata-flush.md)
-   [**\_Начало D3DISSUE**](d3dissue-begin.md)
-   [**D3DISSUE \_ конец**](d3dissue-end.md)
-   [**\_Версия D3DPS**](d3dps-version.md)
-   [**D3DTS \_ мир**](d3dts-world.md)
-   [**D3DTS \_ ворлдматрикс**](d3dts-worldmatrix.md)
-   [**D3DTS \_ ворлдн**](d3dts-worldn.md)
-   [**\_Версия D3DVS**](d3dvs-version.md)

### <a name="requirements"></a>Требования

Все макросы Direct3D имеют следующие требования.



| Требование | Значение |
|--------|-------------|
| Заголовок | d3d9types. h |



 

## <a name="related-topics"></a>Связанные темы

<dl> <dt>

[Справочник по Direct3D](dx9-graphics-reference-d3d.md)
</dt> </dl>

 

 



